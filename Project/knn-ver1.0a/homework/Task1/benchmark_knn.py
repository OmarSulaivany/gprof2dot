...
import os
import glob
import time  # Import time module for precise execution time measurement
...
for opt in optimizations:
    ...
    for i in range(num_runs):
        ...
        subprocess.run(f"{PROJECT_DIR}/knn", shell=True, cwd=PROJECT_DIR)
    ...
    # Merge all gmon.out files into one for accurate profiling
    ...
    # Generate call graph using merged profiling data
    print(f"  Generating call graph for {opt}...")
    subprocess.run(f"gprof {PROJECT_DIR}/knn {' '.join([f'{OUTPUT_DIR}/gmon_{opt}_{i+1}.out' for i in range(num_runs)])} | gprof2dot | dot -Tpng -o {OUTPUT_DIR}/callgraph_{opt}.png", shell=True, check=True)
